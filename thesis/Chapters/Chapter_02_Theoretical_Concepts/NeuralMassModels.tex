...
%\textbf{Remark} (Convolution).\textit{
...
\footnotetext{There is a very intuitive explanation of convolutions by Kalid Azad on his website \url{https://betterexplained.com/articles/intuitive-convolution/}}
...
The simulated data from $y_1-y_2$ while varying $C$ is plotted in Fig.~\ref{fig:jr_c_sweep}:
\begin{figure}[H]
    \centering
    \subfloat[\textbf{JR Model Output for varying $C$.} Well defined alpha-activity is visible at $C=135$.]{\label{fig:jr_c_sweep}
        \resizebox{!}{13cm}{
            \begin{tikzpicture}
                \pgfplotsset{
                %% Axis
    ...
    \hfill
    \subfloat[\textbf{Spectrogram of JR Model Output} ($C=135$)]{\label{fig:jr_spec}
        \resizebox{9.5cm}{!}{
...
David and Friston slightly redefine the impulse response function $h(t)$ in the PSP-Blocks by introducing the
...
%            \addplot[magenta, dashed] {(151.72/4.35)*x*e^(-(1/4.35)*x)};\label{plot:line6.6}
    ...
    subpopulations. They contribute to activity with higher frequencies}
    \label{fig:PSPPlotDavidFriston}
\end{figure}

\subsubsection{Model Output}
\todo{explain output of DF model!}
\begin{figure}[H]
    \centering
    ...
    \hfill
    \subfloat[\textbf{Spectrogram of JR Model Output.} Scaled to match intensity for comparison]{\label{fig:jr_spec_rescaled}
        \resizebox{9.5cm}{!}{
            \import{data/rest_sim}{JR_REST_60_dfscale.pgf}
        }
    }
    \label{fig:df_model_out_props}
    \caption{\textbf{Properties of DF Model Output}}
\end{figure}